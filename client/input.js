...
module.exports = class Input {
    ...
    static rtAudio = new RtAudio();
    static client = dgram.createSocket('udp4');
    static streamOpen = false;
    static address;
    static aes67Multicast;
    static inputDevice;
    static name;
    static samplerate;
    ...
    static start() {
        ...
        if (selectedDevice && selectedDevice.inputChannels > 0) {
            ...
            this.audioChannels = Math.min(8, selectedDevice.inputChannels);
        } else {
        ...
        ptpv2.init(this.address, 0, function () {
            ...
            that.rtAudio.start();
            sdp.start(that.address, that.aes67Multicast, that.samplerate, that.audioChannels, encoding, that.name, sessID, sessVersion, that.ptpMaster);
